...
from cleandiffuser.dataset.d4rl_antmaze_dataset import D4RLAntmazeTDDataset_minari
...
def pipeline(args):
    ...
    env_id = env.spec.id
    dataset = D4RLAntmazeTDDataset_minari(dataset)
    ...
    # ---------------------- Inference ----------------------
        ...
        for i in range(args.num_episodes):
            ...
            while not np.all(cum_done) and t < 1000 + 1:
                ...
                act_list.append(sampled_act)
                # step
                obs, rew, terminations, truncations, info = env_eval.step(sampled_act)
                obs = np.concatenate([obs['achieved_goal'], obs['observation'], obs['desired_goal']], axis=-1)
                ...
                if t % 400 == 0:
                    ...
                    print(f'[t={t}] cum_rew: {ep_reward}, time: {datetime.datetime.now()}')
